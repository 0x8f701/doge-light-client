...
use doge_light_client::{
    common_types::QHash256, hash::sha256::QBTCHash256Hasher, network_params::DogeNetworkType,
};
use psy_doge_data_link::{
    link_common::wrapped_hash_256::WrappedHash256, link_sync::electrs_link::DogeLinkElectrsClient,
    traits::QDogeBlockHeaderFetcher,
};
fn run_merkle_test(txids: &[QHash256], client: DogeLinkElectrsClient) -> anyhow::Result<()> {
    for txid in txids.iter() {
        ...
        println!(
            ...
            WrappedHash256(q_proof.root).to_hex_string(),
            WrappedHash256(blk.header.merkle_root).to_hex_string()
        );
        ...
        assert!(
            q_proof.verify_btc_block_tx_tree::<QBTCHash256Hasher>(),
        ...
        for _ in 0..256 {
            ...
            assert!(
                !q_proof.verify_btc_block_tx_tree::<QBTCHash256Hasher>(),
                "merkle proof verification should have failed if we changed the index"
            );
        }
    }
    Ok(())
