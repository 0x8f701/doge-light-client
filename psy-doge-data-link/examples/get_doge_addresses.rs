...
use doge_light_client::{
    constants::DogeTestNetConfig,
    doge::{address::BTCAddress160, transaction::BTCTransaction},
...
fn get_doge_output_addresses_for_tx(tx: &BTCTransaction) -> Vec<BTCAddress160> {
    let mut addrs = tx
        .outputs
        .iter()
        .map(|x| x.get_output_address())
        .filter(|x| x.is_ok())
        .map(|x| x.unwrap())
        .collect::<Vec<BTCAddress160>>();
    addrs.sort();
    addrs.dedup();
    addrs
}
            ...
            for tx in block.transactions.iter() {
                ...
                for addr in addrs.iter() {
                    if output_addresses.insert(addr.to_owned()) {
                        println!(
                            "new address: {}",
                            addr.to_address_string::<DogeTestNetConfig>()
                        );
                    } else {
                        println!(
                            "old address: {}",
