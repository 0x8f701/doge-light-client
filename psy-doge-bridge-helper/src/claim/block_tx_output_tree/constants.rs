...
pub const TXO_TREE_COMBINED_INDEX_BITS_OUTPUT_NUM_LENGTH: usize =
    TXO_TREE_INDEX_BITS_TOP_OUTPUT_NUM_LENGTH + TXO_TREE_LEAF_BIT_INDEX_LENGTH;

pub const TXO_MERKLE_INDEX_TOTAL_BITS: usize = TXO_TREE_INDEX_BITS_BLOCK_NUM_LENGTH
...
pub const TXO_COMBINED_INDEX_TOTAL_BITS: usize = TXO_TREE_INDEX_BITS_BLOCK_NUM_LENGTH
    + TXO_TREE_INDEX_BITS_TX_NUM_LENGTH
...
pub const TXO_TREE_MAX_OUTPUTS_PER_TX: u16 =
    (1u16 << (TXO_TREE_COMBINED_INDEX_BITS_OUTPUT_NUM_LENGTH)) - 1;

pub const TXO_TREE_INDEX_BLOCK_NUM_LOWERED_MASK: u64 =
    (1u64 << TXO_TREE_INDEX_BITS_BLOCK_NUM_LENGTH) - 1;
pub const TXO_TREE_INDEX_TX_NUM_LOWERED_MASK: u64 = (1u64 << TXO_TREE_INDEX_BITS_TX_NUM_LENGTH) - 1;
pub const TXO_TREE_INDEX_OUTPUT_NUM_LOWERED_MASK: u64 =
    (1u64 << TXO_TREE_COMBINED_INDEX_BITS_OUTPUT_NUM_LENGTH) - 1;

pub const TXO_FULL_MERKLE_TREE_HEIGHT: usize = TXO_MERKLE_INDEX_TOTAL_BITS;
pub const TXO_BLOCK_FULL_MERKLE_TREE_HEIGHT: usize =
    TXO_TREE_INDEX_BITS_TX_NUM_LENGTH + TXO_TREE_INDEX_BITS_TOP_OUTPUT_NUM_LENGTH;
pub const TXO_TRANSACTION_FULL_MERKLE_TREE_HEIGHT: usize =
    TXO_TREE_INDEX_BITS_TOP_OUTPUT_NUM_LENGTH;

pub const TXO_EMPTY_MERKLE_TREE_ROOT: QHash256 = SHA256_ZERO_HASHES[TXO_FULL_MERKLE_TREE_HEIGHT];
pub const TXO_EMPTY_BLOCK_MERKLE_TREE_ROOT: QHash256 =
    SHA256_ZERO_HASHES[TXO_BLOCK_FULL_MERKLE_TREE_HEIGHT];
pub const TXO_EMPTY_TRANSACTION_MERKLE_TREE_ROOT: QHash256 =
    SHA256_ZERO_HASHES[TXO_TRANSACTION_FULL_MERKLE_TREE_HEIGHT];
...
pub const fn get_txo_block_number_tx_number_output_leaf_merkle_index(index: u64) -> (u32, u16, u8) {
    let block_num = ((index
        >> (TXO_TREE_INDEX_BITS_TX_NUM_LENGTH + TXO_TREE_COMBINED_INDEX_BITS_OUTPUT_NUM_LENGTH))
        & TXO_TREE_INDEX_BLOCK_NUM_LOWERED_MASK) as u32;
    let tx_num = ((index >> TXO_TREE_COMBINED_INDEX_BITS_OUTPUT_NUM_LENGTH)
        & TXO_TREE_INDEX_TX_NUM_LOWERED_MASK) as u16;
...
pub const fn get_txo_merkle_index(block_num: u32, tx_num: u16, output_leaf_index: u8) -> u64 {
    ((block_num as u64)
        << (TXO_TREE_INDEX_BITS_TX_NUM_LENGTH + TXO_TREE_INDEX_BITS_TOP_OUTPUT_NUM_LENGTH))
        | ((tx_num as u64) << TXO_TREE_INDEX_BITS_TOP_OUTPUT_NUM_LENGTH)
        | (output_leaf_index as u64)
}

#[inline(always)]
pub const fn get_txo_merkle_index_with_output_index(
    block_num: u32,
    tx_num: u16,
    output_index_in_tx: u16,
) -> u64 {
    get_txo_merkle_index(block_num, tx_num, (output_index_in_tx >> 8) as u8)
}
#[inline(always)]
pub const fn get_txo_merkle_index_and_bit_index(
    block_num: u32,
    tx_num: u16,
    output_index_in_tx: u16,
) -> (u64, u8) {
...
#[inline(always)]
pub const fn get_txo_combined_index_for_merkle_index_bit_index(
    block_num: u32,
    tx_num: u16,
    output_merkle_index: u8,
    output_bit_index: u8,
) -> u64 {
    ((block_num as u64)
        << (TXO_TREE_INDEX_BITS_TX_NUM_LENGTH + TXO_TREE_COMBINED_INDEX_BITS_OUTPUT_NUM_LENGTH))
...
#[inline(always)]
pub const fn get_txo_block_number_tx_number_output_index_from_combined_index(
    index: u64,
) -> (u32, u16, u16) {
    let block_num = ((index
        >> (TXO_TREE_INDEX_BITS_TX_NUM_LENGTH + TXO_TREE_COMBINED_INDEX_BITS_OUTPUT_NUM_LENGTH))
...
#[inline(always)]
pub const fn is_valid_block_num_tx_num_output_index(
    block_num: u32,
    tx_num: u16,
    output_index_in_tx: u16,
) -> bool {
    (block_num & (TXO_TREE_INDEX_BLOCK_NUM_LOWERED_MASK as u32)) == block_num
        ...
        && (output_index_in_tx & (TXO_TREE_INDEX_OUTPUT_NUM_LOWERED_MASK as u16))
            == output_index_in_tx
}
...
#[inline(always)]
pub const fn get_output_in_tx_merkle_index_bit_index_byte_index_bit_mask(
    output_index_in_tx: u16,
) -> (u8, u8, u8, u8) {
    let merkle_index = (output_index_in_tx >> TXO_TREE_LEAF_BIT_INDEX_LENGTH) as u8;
    let bit_index =
        (output_index_in_tx - ((merkle_index as u16) << TXO_TREE_LEAF_BIT_INDEX_LENGTH)) as u8;
...
mod tests {
    ...
    fn test_txo_index_functions() {
        ...
        assert!(is_valid_block_num_tx_num_output_index(
            block_num,
            tx_num,
            output_index_in_tx
        ));
        ...
        let merkle_index =
            get_txo_merkle_index_with_output_index(block_num, tx_num, output_index_in_tx);
        ...
        // round trip
        let combined_index_2 = get_txo_combined_index_for_merkle_index_bit_index(
            block_num,
            tx_num,
            (output_index_in_tx >> 8) as u8,
            (output_index_in_tx & 0xFF) as u8,
        );
