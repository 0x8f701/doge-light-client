...
use crate::{
    ...
    hash::traits::{get_zero_hashes_sized, MerkleHasher, MerkleZeroHasher, ZeroableHash},
};

use super::{
    append_tree::MerkleAppendTreeLevel,
...
#[cfg(feature = "serialize_serde")]
impl<Hash: PartialEq + Copy, const HEIGHT: usize> serde::Serialize
    for FixedMerkleAppendTree<Hash, HEIGHT>
where
    Hash: serde::Serialize + serde::de::DeserializeOwned,
{
...
#[cfg(feature = "serialize_serde")]
impl<'de, Hash: PartialEq + Copy, const HEIGHT: usize> serde::Deserialize<'de>
    for FixedMerkleAppendTree<Hash, HEIGHT>
where
    Hash: serde::Serialize + serde::de::DeserializeOwned,
{
    ...
    {
        let SerFixedMerkleAppendTree { next_index, levels } =
            serde::Deserialize::deserialize(deserializer)?;
        Ok(Self { next_index, levels })
    }
}

impl<Hash: PartialEq + Copy + ZeroableHash, const HEIGHT: usize>
    FixedMerkleAppendTree<Hash, HEIGHT>
{
...
impl<Hash: PartialEq + Copy, const HEIGHT: usize> FixedMerkleAppendTree<Hash, HEIGHT> {
    ...
    pub fn revert_to_index<Hasher: MerkleHasher<Hash>>(
        &mut self,
        index: u64,
        changed_left_siblings: &[Hash],
        value: Hash,
    ) -> QDogeResult<()> {
